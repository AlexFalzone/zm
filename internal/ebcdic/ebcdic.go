package ebcdic

// IBM-1047 (z/OS default EBCDIC) ↔ ISO-8859-1 (Latin-1/ASCII) conversion.

var toASCIITable = [256]byte{
	,
}

var toEBCDICTable [256]byte

func init() {
	for i := range toEBCDICTable {
		toEBCDICTable[i] = 0x40 // default to EBCDIC space for unmapped
	}
	for ebcdic, ascii := range toASCIITable {
		toEBCDICTable[ascii] = byte(ebcdic)
	}
}

// IsEBCDIC returns true if data looks like IBM-1047 EBCDIC rather than ASCII.
func IsEBCDIC(data []byte) bool {
	if len(data) == 0 {
		return false
	}

	limit := len(data)
	if limit > 512 {
		limit = 512
	}
	sample := data[:limit]

	var ebcdicSpaces, asciiSpaces, asciiNewlines int
	for _, b := range sample {
		switch b {
		case 0x40: // EBCDIC space
			ebcdicSpaces++
		case 0x20: // ASCII space
			asciiSpaces++
		case 0x0A: // ASCII newline
			asciiNewlines++
		}
	}

	// If we see EBCDIC spaces but few ASCII spaces/newlines → EBCDIC
	return ebcdicSpaces > 0 && asciiSpaces == 0 && asciiNewlines == 0
}

// ToASCII converts IBM-1047 EBCDIC bytes to ASCII/Latin-1.
func ToASCII(data []byte) []byte {
	out := make([]byte, len(data))
	for i, b := range data {
		out[i] = toASCIITable[b]
	}
	return out
}

// ConvertToASCII converts IBM-1047 EBCDIC bytes to ASCII/Latin-1 in-place.
func ConvertToASCII(data []byte) {
	for i, b := range data {
		data[i] = toASCIITable[b]
	}
}

// ToEBCDIC converts ASCII/Latin-1 bytes to IBM-1047 EBCDIC.
func ToEBCDIC(data []byte) []byte {
	out := make([]byte, len(data))
	for i, b := range data {
		out[i] = toEBCDICTable[b]
	}
	return out
}
